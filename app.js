...
'use strict';
console.log('http://philippetring.fr');
...
var words 		= ["Easiware", "Ninja du CRM"]
var particles 	= [];
var density 	= 15;
var end;
var start = function() {
  	canvas.context.font = "bold 170px Arial";
  	canvas.context.fillText(words[0], ( canvasW / 2 ) - ( Math.round( context.measureText(words[0]).width/2 ) ), canvasH / 2);
  	canvas.context.fillText(words[1], ( canvasW / 2 ) - ( Math.round( context.measureText(words[1]).width/2 ) ), canvasH / 2 + 200);
  	setupShurikens();
  	draw();
}
...
var setupShurikens  = function() {
    ...
    var i       	= 0;
    var slide   	= false;
  	var imageData	= context.getImageData( 0, 0, canvasW, canvasH ).data;
...
canvas.el.addEventListener('imageLoaded', start);
});
